...
def hit_5_accuracy(predicted_output, output_data):
    count = 0
    for i in range(len(predicted_output)):
        current_output = list(predicted_output[i].argsort()[-5:][::-1])
...
data = 'test'
source_input_path = '/Users/PycharmProjects/detect_sport_video/' + data + '_frame_resize_16c/'
source_output_path  = '/Users/PycharmProjects/detect_sport_video/sports-1m-dataset/' + data + '_correct_links.txt'


input_data =np.zeros((test_data_size,sequence_length,dimension))
...
with open(source_output_path) as input_file:
 while cnt <= test_data_size:
        output_classes.append(int(input_file.readline().split()[2]))
        cnt += 1
output_data =np.zeros((test_data_size,classes))
output_index = 0
while(output_index < test_data_size):
    output_data[output_index,output_classes[output_index]] = 1
    output_index += 1


model = model_from_json(open('model_16c_final.json').read())
model.load_weights('model_16c_final.h5')
