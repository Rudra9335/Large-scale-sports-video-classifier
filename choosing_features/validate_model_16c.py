...
def hit_5_accuracy(predicted_output, output_data):
        ...
        for i in range(len(predicted_output)):
            ...
            print len(arr),len(predicted_output),len(predicted_output[i]),len(predicted_output[i][0])
...
model = model_from_json(open('model_16c.json').read())
model.load_weights('model_16c.h5')
